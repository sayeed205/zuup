...
def test_gui_mode():
    ...
    try:
        ...
        console.print("[dim]GUI mode would start here (requires user interaction)[/dim]")
...
def demonstrate_cli_usage():
    ...
    table.add_row(
        "GUI Only",
        "zuup start --gui",
        "GUI with local server"
    )
    table.add_row(
        "Server Only",
        "zuup start --server-only",
        "Headless server for remote access"
    )
    table.add_row(
        "Custom Port",
        "zuup start --port 9000",
        "Custom server port"
    )
    table.add_row(
        "Remote Server",
...
async def main():
    ...
    console.print("[bold green]Zuup Download Manager - Deployment Modes Demo[/bold green]")
    console.print()
