...
async def test_task_management():
    ...
    try:
        ...
        if task1_current.status.value == "downloading":
            ...
            logger.info(f"Task 1 status is {task1_current.status.value}, skipping pause/resume test")
        ...
        for task in final_tasks:
            logger.info(f"Task {task.id}: {task.status.value} - {task.progress.downloaded_bytes} bytes")
