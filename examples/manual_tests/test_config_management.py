...
def test_basic_config_operations():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"✓ Default global config loaded: {global_config.max_concurrent_downloads} concurrent downloads")
        
        # Test default task config
        task_config = manager.get_task_config("test_task_1")
        print(f"✓ Default task config loaded: {task_config.retry_attempts} retry attempts")
...
def test_environment_variable_overrides():
    ...
    try:
        ...
        for var in ["ZUUP_MAX_CONCURRENT_DOWNLOADS", "ZUUP_LOGGING_LEVEL", 
                   "ZUUP_SERVER_PORT", "ZUUP_PROXY_ENABLED", "ZUUP_HTTP_PROXY"]:
