...
class TestDownloadSimulator:
    ...
    async def simulate_download(
        self, duration: float = 10.0, simulate_errors: bool = False
    ) -> None:
        ...
        try:
            while self.running and downloaded < total_bytes:
                ...
                if (
                    simulate_errors
                    and downloaded > total_bytes * 0.3
                    and downloaded < total_bytes * 0.4
                ):
            ...
            if downloaded >= total_bytes:
                ...
                self.logger.log_completion(
                    downloaded, time.time() - start_time, self.task.url
                )

            self.metrics_collector.complete_task_monitoring(
...
def test_basic_logging():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        setup_logging(level="DEBUG", log_file=log_dir / "test.log", rich_console=True)
        ...
        setup_logging(
            level="INFO", log_file=log_dir / "structured.log", structured_logging=True
        )
...
async def test_performance_monitoring():
    ...
    simulators = [TestDownloadSimulator(task, metrics) for task in tasks]

    # Run downloads concurrently
    download_tasks = [
        asyncio.create_task(
            sim.simulate_download(duration=3.0, simulate_errors=(i == 1))
        )
        ...
        try:
            ...
            for i, sim in enumerate(simulators):
                ...
                task = asyncio.create_task(
                    sim.simulate_download(
                        duration=duration, simulate_errors=simulate_errors
                    )
                )
