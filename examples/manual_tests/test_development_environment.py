...
def test_python_version() -> bool:
    ...
    if sys.version_info >= (3, 10):
        print(
            f"✅ Python version is compatible: {sys.version_info.major}.{sys.version_info.minor}"
        )
        return True
    else:
        print(
            f"❌ Python version is too old: {sys.version_info.major}.{sys.version_info.minor}"
        )
        return False
...
def test_dependencies() -> bool:
    ...
    dependencies = [
        ...
        (
            "PySide6",
            ["uv", "run", "python", "-c", "import PySide6; print('PySide6 available')"],
        ),
    ]
...
"""
    ...
    try:
        ...
        success, output = run_test(
            ["uv", "run", "mypy", str(test_file)], "mypy type check"
        )
...
"""
    ...
    try:
        test_file.write_text(test_content)
        success, output = run_test(
            ["uv", "run", "ruff", "check", str(test_file)], "ruff linting check"
        )
        test_file.unlink()  # Clean up

        # Ruff should find issues, so success=False is expected
        if not success and (
            "F401" in output or "unused import" in output.lower()
        ):  # Unused import error
...
"""
    ...
    try:
        ...
        success, output = run_test(
            ["uv", "run", "ruff", "format", str(test_file)], "ruff formatting"
        )
