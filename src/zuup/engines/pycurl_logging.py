...
from ..utils.logging import DownloadLoggerAdapter, get_download_logger
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "transfer": {
                ...
                "efficiency": (self.downloaded_bytes / self.total_bytes * 100) if self.total_bytes > 0 else 0,
            },
            "connection": {
...
class CurlLogger:
    ...
    def __init__(
        ...
        self.logger.debug(f"Initialized CurlLogger for worker {worker_id} with level {log_level.value}")
    
    def set_log_level(self, level: LogLevel) -> None:
        """Update logging level."""
        self.log_level = level
        self.logger.info(f"Updated log level to {level.value} for worker {self.worker_id}")
    ...
    def _write_debug_to_file(self, debug_info: CurlDebugInfo) -> None:
        ...
        try:
            ...
            with open(self.debug_file, "a", encoding="utf-8") as f:
                ...
                f.write(f"[{timestamp}] {self.worker_id} {debug_info.debug_type.name}: ")
                
                if debug_info.debug_type == CurlDebugType.TEXT:
                    f.write(debug_info.data.decode("utf-8", errors="replace").strip())
                elif debug_info.debug_type in (CurlDebugType.HEADER_IN, CurlDebugType.HEADER_OUT):
                    f.write(debug_info.data.decode("utf-8", errors="replace").strip())
                else:
    ...
    def log_connection_failure(self, error: Exception, curl_code: Optional[int] = None) -> None:
    ...
    def log_progress_update(self, progress: WorkerProgress) -> None:
        ...
        if self.log_level in (LogLevel.BASIC, LogLevel.DETAILED, LogLevel.VERBOSE, LogLevel.DEBUG):
    ...
    def collect_performance_metrics(self, curl_handle: pycurl.Curl) -> PerformanceMetrics:
        """
        ...
        try:
            ...
            self.metrics.starttransfer_time = curl_handle.getinfo(pycurl.STARTTRANSFER_TIME)
            ...
            # SSL information
            self.metrics.ssl_verify_result = curl_handle.getinfo(pycurl.SSL_VERIFYRESULT)
...
class CurlDebugUtilities:
    ...
    @staticmethod
    def generate_curl_command(url: str, config: HttpFtpConfig, segment: Optional[DownloadSegment] = None) -> str:
        """
        ...
        if config.proxy.enabled and config.proxy.host:
            ...
            if config.proxy.username and config.proxy.password:
                cmd_parts.extend(["--proxy-user", f"{config.proxy.username}:{config.proxy.password}"])
