...
class MetadataExtractor:
    ...
    def extract_comprehensive_metadata(
        ...
        if yt_dlp_info.get("upload_date"):
            try:
                ...
                self.logger.warning(
                    f"Could not parse upload_date: {yt_dlp_info.get('upload_date')}"
                )
    ...
    def _get_best_thumbnail_url(self, yt_dlp_info: dict[str, Any]) -> str | None:
        """
    ...
    def create_template_variables(
        self, info: MediaInfo, metadata: MediaMetadata
    ) -> dict[str, str]:
        """
        ...
        variables["uploader"] = self._sanitize_for_template(
            metadata.uploader or "Unknown"
        )
        ...
        # Media properties
        variables["duration"] = (
            str(int(metadata.duration)) if metadata.duration else "Unknown"
        )
        variables["view_count"] = (
            str(metadata.view_count) if metadata.view_count else "0"
        )
        variables["like_count"] = (
            str(metadata.like_count) if metadata.like_count else "0"
        )
        ...
        if info.formats:
            ...
            variables["filesize"] = (
                str(first_format.filesize) if first_format.filesize else "Unknown"
            )
        else:
            variables.update(
                {
    ...
    def _sanitize_for_template(self, text: str | None) -> str:
        """
...
class FilenameGenerator:
    ...
    def generate_directory_path(
        self, variables: dict[str, str], base_path: Path
    ) -> Path:
        """
        ...
        # Apply template substitution
        dir_structure = self._substitute_template(
            self.config.directory_template, variables
        )
...
class ThumbnailManager:
    ...
    async def download_thumbnail(self, url: str, output_path: Path) -> Path | None:
        """
...
class MetadataManager:
    ...
    def __init__(
        self,
        template_config: MetadataTemplate | None = None,
    ...
    ) -> tuple[Path, MediaMetadata, Path | None]:
        """
