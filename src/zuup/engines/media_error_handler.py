...
class ExtractionError(Exception):
    ...
    def __init__(
        self, message: str, error_code: str | None = None, extractor: str | None = None
    ):
...
class DownloadError(Exception):
    ...
    def __init__(
        self, message: str, error_code: str | None = None, format_id: str | None = None
    ):
        super().__init__(message)
        self.message = message
        self.error_code = error_code
...
class ProcessingError(Exception):
    ...
    def __init__(
        self, message: str, step: str | None = None, file_path: str | None = None
    ):
        super().__init__(message)
        self.message = message
        self.step = step
...
class MediaErrorHandler:
    ...
    def __init__(
        ...
        self._fallback_extractors: dict[str, str | None] = (
    ...
    async def handle_extraction_error(
        self, error: ExtractionError, url: str, attempt: int = 0
    ) -> MediaErrorAction:
        """
    ...
    async def handle_download_error(
        self, error: DownloadError, info: MediaInfo, attempt: int = 0
    ) -> MediaErrorAction:
        """
    ...
    async def handle_processing_error(
        self, error: ProcessingError, file_path: str, attempt: int = 0
    ) -> MediaErrorAction:
        """
    ...
    def get_fallback_extractor(self, url: str) -> str | None:
        """
    ...
    def suggest_format_alternatives(
        self, failed_format: MediaFormat, available_formats: list[MediaFormat]
    ) -> list[MediaFormat]:
        """
        ...
        candidates.sort(
            key=lambda fmt: self._calculate_format_similarity(failed_format, fmt),
            reverse=True,
        )
    ...
    async def calculate_retry_delay(
        self, attempt: int, error_category: MediaErrorCategory
    ) -> float:
        """
    ...
    def _categorize_extraction_error(
        self, error: ExtractionError, url: str, attempt: int
    ) -> MediaError:
        ...
        if any(
            pattern in message
            for pattern in ["network", "connection", "timeout", "unreachable", "dns"]
        ):
            return MediaError(
        ...
        if any(
            pattern in message
            for pattern in ["unsupported", "no suitable", "extractor"]
        ):
            action = (
                MediaErrorAction.RETRY_WITH_FALLBACK
    ...
    def _categorize_download_error(
        self, error: DownloadError, info: MediaInfo, attempt: int
    ) -> MediaError:
        ...
        if any(
            pattern in message
            for pattern in ["network", "connection", "timeout", "reset", "broken pipe"]
        ):
            return MediaError(
                category=MediaErrorCategory.NETWORK,
                action=MediaErrorAction.RETRY_WITH_DELAY,
        ...
        if any(
            pattern in message
            for pattern in ["format", "codec", "not available", "expired", "manifest"]
        ):
            return MediaError(
        ...
        if any(pattern in message for pattern in ["403", "404", "410", "http"]):
            return MediaError(
                category=MediaErrorCategory.FORMAT,
    ...
    def _categorize_processing_error(
        self, error: ProcessingError, file_path: str, attempt: int
    ) -> MediaError:
        ...
        if any(
            pattern in message
            for pattern in ["ffmpeg", "conversion", "codec", "format", "encoding"]
        ):
            return MediaError(
                category=MediaErrorCategory.PROCESSING,
                action=MediaErrorAction.SKIP_ITEM
                if attempt > 1
                else MediaErrorAction.RETRY,
                message=error.message,
                original_error=str(error),
                url=file_path,
                retry_count=attempt,
        ...
        if any(
            pattern in message
            for pattern in ["permission", "disk", "space", "write", "read", "file"]
        ):
            return MediaError(
                category=MediaErrorCategory.PROCESSING,
                action=MediaErrorAction.FAIL_DOWNLOAD,
        ...
        return MediaError(
            category=MediaErrorCategory.PROCESSING,
            action=MediaErrorAction.RETRY
            if attempt < 2
            else MediaErrorAction.SKIP_ITEM,
            message=error.message,
            original_error=str(error),
            url=file_path,
            retry_count=attempt,
    ...
    def _calculate_format_similarity(
        self, failed_format: MediaFormat, candidate: MediaFormat
    ) -> float:
    ...
    def _initialize_fallback_extractors(self) -> dict[str, str | None]:
