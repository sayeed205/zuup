...
class ErrorHandler:
    ...
    def __init__(
        ...
        self._connection_errors: dict[
            str, int
        ] = {}  # task_id -> connection_error_count
        self._last_error_times: dict[str, float] = {}  # task_id -> timestamp

        # Connection reduction tracking
        self._reduced_connections: dict[
            str, int
        ] = {}  # task_id -> reduced_connection_count
    ...
    async def handle_error(
        ...
        logger: DownloadLoggerAdapter | None = None,
    ) -> ErrorAction:
        """
    ...
    def should_retry(
        self, error: CurlError, attempt: int, context: ErrorContext
    ) -> bool:
        """
    ...
    def get_reduced_connection_count(
        self, task_id: str, current_connections: int
    ) -> int:
        """
    ...
    def _log_error(
        ...
        error: CurlError,
        context: ErrorContext,
        logger: DownloadLoggerAdapter | None,
    ) -> None:
    ...
    def _update_error_tracking(self, task_id: str, error: CurlError) -> None:
        ...
        if error.category in (
            ...
            if (
                ...
                and error.curl_code == pycurl.E_TOO_MANY_REDIRECTS
            ) or error.category in (ErrorCategory.NETWORK, ErrorCategory.TIMEOUT):
                self._connection_errors[task_id] = (
                    self._connection_errors.get(task_id, 0) + 1
                )

        # Update last error time
        self._last_error_times[task_id] = current_time

    def _determine_error_action(
        self, error: CurlError, context: ErrorContext
    ) -> ErrorAction:
...
class ConnectionReductionStrategy:
    ...
    def apply_server_hints(
        self, current_connections: int, server_hints: dict[str, Any]
    ) -> int:
        """
