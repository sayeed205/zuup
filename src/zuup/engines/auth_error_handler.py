...
class AuthenticationErrorHandler:
    ...
    def __init__(self, auth_manager: AuthenticationManager) -> None:
        ...
        self._error_patterns = {
            AuthErrorType.INVALID_CREDENTIALS: [
                ...
                "authentication failed", "unauthorized", "403", "401"
            ],
            AuthErrorType.EXPIRED_SESSION: [
                "session expired", "session invalid", "cookie expired",
                "please log in", "login required"
            ],
            AuthErrorType.MISSING_COOKIES: [
                "no cookies", "cookie required", "session required",
                "please enable cookies"
            ],
            AuthErrorType.OAUTH_TOKEN_EXPIRED: [
                "token expired", "invalid token", "token revoked",
    ...
    def classify_error(self, error: Exception, auth_config: AuthConfig) -> AuthErrorType:
        """
        ...
        elif auth_config.method == AuthMethod.USERNAME_PASSWORD and "password" in error_message:
    ...
    async def _try_fallback_auth(self, auth_config: AuthConfig, url: str) -> Optional[Dict[str, Any]]:
        """
        ...
        if auth_config.method == AuthMethod.USERNAME_PASSWORD:
            stored_creds = await self.auth_manager.credential_manager.retrieve_credentials(domain)
            if stored_creds:
        ...
        if auth_config.method != AuthMethod.COOKIES:
            browser_cookies = await self.auth_manager.cookie_manager.get_cookies_for_domain(domain)
            if browser_cookies:
                ...
                if temp_cookies_file:
                    await self.auth_manager.cookie_manager.save_cookies(browser_cookies, domain)
    ...
    async def _switch_auth_method(self, auth_config: AuthConfig, url: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def _auth_config_to_dict(self, auth_config: AuthConfig) -> Dict[str, Any]:
        ...
        return {
            ...
            "cookies_file": str(auth_config.cookies_file) if auth_config.cookies_file else None,
            "netrc_file": str(auth_config.netrc_file) if auth_config.netrc_file else None,
