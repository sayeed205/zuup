...
from .media_models import MediaFormat, FormatPreferences
...
class FormatSelector:
    ...
    def select_optimal_format(
        ...
        target_quality: Optional[QualityTier] = None,
        adaptive: bool = True,
        ...
        if target_quality is None and adaptive:
            target_quality = self._determine_optimal_quality(filtered_formats, connection_speed)
        
        # Apply intelligent quality-based filtering
        quality_filtered = self._apply_quality_based_filtering(filtered_formats, preferences, target_quality)
        
        if not quality_filtered:
            logger.warning("Quality-based filtering removed all formats, using filtered formats")
            quality_filtered = filtered_formats
        
        # Score and rank formats
        scored_formats = self._score_formats(quality_filtered, preferences, target_quality, connection_speed)
        
        # Select best format
        best_format = max(scored_formats, key=lambda x: x[1])[0]
        
        logger.info(f"Selected format: {best_format.format_id} "
    ...
    def select_audio_only_format(
        ...
        filtered_formats = self._filter_audio_formats(audio_formats, preferences, target_bitrate)
        
        if not filtered_formats:
            filtered_formats = audio_formats
        
        # Score audio formats with conversion consideration
        scored_formats = self._score_audio_formats(filtered_formats, preferences, target_bitrate, allow_conversion)
        
        best_format = max(scored_formats, key=lambda x: x[1])[0]
        
        logger.info(f"Selected audio format: {best_format.format_id} "
    ...
    def get_quality_alternatives(
        ...
        for fmt in formats:
            ...
            if direction == "lower" and fmt_height < current_height:
                alternatives.append(fmt)
            elif direction == "higher" and fmt_height > current_height:
                alternatives.append(fmt)
        
        # Sort by quality (ascending for lower, descending for higher)
        reverse_sort = direction == "higher"
        alternatives.sort(
            key=lambda x: self._get_format_height(x) or 0,
    ...
    def _apply_preference_filters(
        ...
        if preferences.max_height:
            filtered = [
                f for f in filtered
                if self._get_format_height(f) is None or 
                   self._get_format_height(f) <= preferences.max_height
            ]
        
        if preferences.max_width:
            filtered = [
                f for f in filtered
                if self._get_format_width(f) is None or
                   self._get_format_width(f) <= preferences.max_width
            ]
    ...
    def _filter_audio_formats(
        ...
        if target_bitrate:
            bitrate_filtered = [
                f for f in filtered
                if f.abr is not None and abs(f.abr - target_bitrate) <= target_bitrate * 0.2
            ]
            if bitrate_filtered:
        ...
        if preferences.preferred_codecs:
            codec_filtered = [
                f for f in filtered
                if f.acodec and any(codec in f.acodec for codec in preferences.preferred_codecs)
            ]
            if codec_filtered:
                filtered = codec_filtered
    ...
    def _score_formats(
        ...
        formats: List[MediaFormat],
        preferences: FormatPreferences,
        target_quality: Optional[QualityTier],
        connection_speed: Optional[float] = None
    ...
    def _apply_quality_based_filtering(
        ...
        if preferences.max_fps:
            filtered = [
                f for f in filtered
                if f.fps is None or f.fps <= preferences.max_fps
            ]
        
        if preferences.min_fps:
            filtered = [
                f for f in filtered
                if f.fps is None or f.fps >= preferences.min_fps
            ]
        ...
        if preferences.prefer_hardware_decodable:
            ...
            hw_formats = [
                f for f in filtered
                if f.vcodec and any(codec in f.vcodec.lower() for codec in hw_decodable_codecs)
            ]
            if hw_formats:
        ...
        if preferences.avoid_experimental_codecs:
            ...
            stable_formats = [
                f for f in filtered
                if not f.vcodec or not any(codec in f.vcodec.lower() for codec in experimental_codecs)
            ]
            if stable_formats:
    ...
    def _filter_by_quality_tier(
        ...
        for fmt in formats:
            ...
            if height:
                ...
                if target_quality == QualityTier.ULTRA_HIGH and height >= 1440:
                    tolerance_formats.append(fmt)
                elif target_quality == QualityTier.HIGH and 720 <= height <= 2880:
                    tolerance_formats.append(fmt)
                elif target_quality == QualityTier.MEDIUM and 480 <= height <= 1440:
                    tolerance_formats.append(fmt)
                elif target_quality == QualityTier.LOW and 360 <= height <= 900:
                    tolerance_formats.append(fmt)
                elif target_quality == QualityTier.VERY_LOW and height <= 720:
                    tolerance_formats.append(fmt)
    ...
    def _determine_optimal_quality(
        ...
        heights = [self._get_format_height(f) for f in formats if self._get_format_height(f)]
        ...
        if connection_speed:
            ...
            if connection_speed >= 10 * 1024 * 1024:  # >= 10 MB/s
                target_by_speed = QualityTier.ULTRA_HIGH if max_height >= 2160 else QualityTier.HIGH
            ...
            return min(available_quality, target_by_speed, key=lambda x: list(QualityTier).index(x))
    ...
    def _calculate_codec_score(self, fmt: MediaFormat, preferences: FormatPreferences) -> float:
    ...
    def _calculate_container_score(self, fmt: MediaFormat, preferences: FormatPreferences) -> float:
    ...
    def _calculate_connection_speed_score(self, fmt: MediaFormat, connection_speed: float) -> float:
        ...
        estimated_time = filesize / connection_speed if connection_speed > 0 else float('inf')
    ...
    def _calculate_advanced_preference_score(self, fmt: MediaFormat, preferences: FormatPreferences) -> float:
        ...
        if fmt.fps:
            if preferences.prefer_60fps and fmt.fps >= 60:
                score += 15.0
            elif preferences.max_fps and fmt.fps <= preferences.max_fps:
                score += 5.0
            elif preferences.min_fps and fmt.fps >= preferences.min_fps:
                score += 5.0
    ...
    def _get_format_height(self, fmt: MediaFormat) -> Optional[int]:
    ...
    def _get_format_width(self, fmt: MediaFormat) -> Optional[int]:
    ...
    def _format_has_preferred_codec(self, fmt: MediaFormat, preferred_codecs: List[str]) -> bool:
    ...
    def select_adaptive_quality_format(
        ...
        logger.info(f"Performing adaptive quality selection from {len(formats)} formats")
        ...
        logger.info(f"Adaptive quality target: {adaptive_quality.value if adaptive_quality else 'auto'}")
        ...
        try:
            ...
            logger.warning(f"Adaptive selection failed: {e}, falling back to standard selection")
    ...
    def _create_adaptive_preferences(
        ...
        available_bandwidth: Optional[float],
        device_capabilities: Optional[Dict[str, any]],
        ...
        if available_bandwidth:
            if available_bandwidth < 1 * 1024 * 1024:  # < 1 MB/s
                ...
                adaptive_prefs.max_video_bitrate = min(adaptive_prefs.max_video_bitrate or 2000, 1000)
                ...
                adaptive_prefs.max_video_bitrate = min(adaptive_prefs.max_video_bitrate or 5000, 3000)
        ...
        if device_capabilities:
            ...
            if screen_height:
                ...
                adaptive_prefs.max_height = min(adaptive_prefs.max_height or screen_height, screen_height)
            
            supported_codecs = device_capabilities.get("supported_codecs", [])
            if supported_codecs:
                # Filter to supported codecs
                adaptive_prefs.preferred_codecs = [
                    codec for codec in (adaptive_prefs.preferred_codecs or [])
    ...
    def _determine_adaptive_quality_target(
        ...
        if target_formats_count <= 1:
            logger.info(f"Limited formats in {base_quality.value} quality, enabling adaptive fallback")
            # Return None to enable more flexible selection
            return None
        
        return base_quality

    def get_format_conversion_requirements(self, selected_format: MediaFormat, target_preferences: FormatPreferences) -> Dict[str, any]:
        """
        ...
        if target_preferences.audio_only and selected_format.vcodec and selected_format.vcodec != "none":
            ...
            if target_preferences.preferred_containers:
                audio_containers = [c for c in target_preferences.preferred_containers if c in ["mp3", "m4a", "ogg", "flac"]]
                if audio_containers:
        ...
        if (target_preferences.preferred_containers and 
            selected_format.ext not in target_preferences.preferred_containers):
            
            conversion_info["needs_conversion"] = True
            if conversion_info["conversion_type"] is None:
                conversion_info["conversion_type"] = "container_conversion"
            conversion_info["target_container"] = target_preferences.preferred_containers[0]
            conversion_info["estimated_quality_loss"] = "minimal"
        ...
        if (target_preferences.preferred_codecs and selected_format.vcodec and
            not any(codec in selected_format.vcodec for codec in target_preferences.preferred_codecs)):
            needs_video_conversion = True
            conversion_info["target_video_codec"] = target_preferences.preferred_codecs[0]
        
        if (target_preferences.preferred_codecs and selected_format.acodec and
            not any(codec in selected_format.acodec for codec in target_preferences.preferred_codecs)):
        ...
        if needs_video_conversion or needs_audio_conversion:
            ...
            conversion_info["estimated_quality_loss"] = "moderate" if needs_video_conversion else "minimal"
    ...
    def analyze_format_distribution(self, formats: List[MediaFormat]) -> Dict[str, any]:
        ...
        for fmt in formats:
            ...
            if height:
                ...
                analysis["quality_distribution"][tier] = analysis["quality_distribution"].get(tier, 0) + 1
