...
class BandwidthAllocation:
    ...
    def __init__(
        ...
        # Usage tracking
        self.actual_usage = 0
        self.usage_history: deque[tuple[float, int]] = deque(
            maxlen=60
        )  # 1 minute of history
    ...
    def get_average_usage(self, window_seconds: int = 30) -> float:
        ...
        recent_usage = [
            usage for timestamp, usage in self.usage_history if timestamp >= cutoff_time
        ]
...
class EnhancedBandwidthManager:
    ...
    def add_worker(
        ...
        logger.debug(
            f"Added worker {worker_id} with initial allocation {initial_allocation} B/s"
        )
    ...
    def allocate_bandwidth(self, worker_speeds: dict[str, float]) -> dict[str, int]:
        ...
        if current_time - self.last_update < self.update_interval:
            ...
            return {
                wid: alloc.allocated_bandwidth
                for wid, alloc in self.allocations.items()
            }
        ...
        logger.debug(
            f"Allocated bandwidth using {self.algorithm.value}: {new_allocations}"
        )
    ...
    def _weighted_fair_allocation(
        ...
        for worker_id in worker_speeds:
            ...
            allocation = max(
                self.min_allocation, int(available * weight / total_weight)
            )
            allocations[worker_id] = allocation
    ...
    def _priority_based_allocation(
        ...
        for priority in sorted(
            priority_groups.keys(), key=lambda p: p.value, reverse=True
        ):
            ...
            group_allocation = (
                remaining // 2 if priority != WorkerPriority.LOW else remaining
            )
    ...
    def _adaptive_allocation(self, worker_speeds: dict[str, float]) -> dict[str, int]:
        ...
        for worker_id in worker_speeds:
            if worker_id in self.allocations:
                ...
                consistency = 1.0 - abs(allocation.actual_usage - avg_usage) / max(
                    1, avg_usage
                )
        ...
        for worker_id, score in efficiency_scores.items():
            ...
            if current_speed > 0:
                speed_factor = min(
                    2.0, math.log10(current_speed / 1024 + 1)
                )  # Log scale
    ...
    def _record_performance_metrics(
        ...
        metrics = {
            ...
            "utilization": total_speed / max(1, total_allocated)
        ...
        for worker_id in worker_speeds:
            if worker_id in self.allocations:
                allocation = self.allocations[worker_id]
                metrics["efficiency_scores"][worker_id] = (
                    allocation.get_utilization_rate()
                )
    ...
    def get_allocation_stats(self) -> dict[str, Any]:
        ...
        for worker_id, allocation in self.allocations.items():
            worker_stats = {
                ...
                "priority": self.worker_priorities.get(
                    worker_id, WorkerPriority.NORMAL
                ).name,
                "weight": self.worker_weights.get(worker_id, 1.0),
    ...
    def optimize_allocation(self) -> dict[str, Any]:
        ...
        if overall_util < 0.5:
            suggestions.append(
                "Consider reducing total bandwidth limit or switching to proportional allocation"
            )
        elif overall_util > 0.9:
            suggestions.append(
                "Consider increasing total bandwidth limit or using priority-based allocation"
            )
        ...
        if underutilized_workers:
            suggestions.append(
                f"Workers {underutilized_workers} are underutilized - consider reducing their allocation"
            )

        if overutilized_workers:
            suggestions.append(
                f"Workers {overutilized_workers} are overutilized - consider increasing their allocation"
            )

        # Algorithm recommendations
        if (
            self.algorithm == AllocationAlgorithm.EQUAL
            and len(set(w["utilization_rate"] for w in stats["workers"].values())) > 1
        ):
            suggestions.append(
                "Consider switching to adaptive or proportional allocation for better efficiency"
            )
