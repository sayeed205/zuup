...
from .media_models import MediaFormat, FormatPreferences, DownloadProgress, DownloadStatus
...
class QualityController:
    ...
    def should_adapt_quality(
        ...
        if (current_progress.download_speed is not None and
            current_progress.download_speed < self.min_speed_threshold):
            ...
            if lower_quality_formats:
                logger.info(f"Slow speed detected ({current_progress.download_speed:.0f} B/s), "
    ...
    def adapt_quality(
        ...
        logger.info(f"Quality adapted from {current_format.format_id} to "
    ...
    def analyze_download_performance(
        self,
        download_id: str,
        progress_history: List[DownloadProgress]
    ) -> Dict[str, float]:
        """
        ...
        completed_progress = [p for p in progress_history if p.status == DownloadStatus.FINISHED]
    ...
    def get_recommended_quality_for_connection(
        ...
        Returns:
            Recommended format
        """
        logger.info(f"Recommending quality for connection speed: {connection_speed:.0f} B/s")
        ...
        try:
            return self.format_selector.select_optimal_format(
                available_formats, speed_prefs, target_quality, 
                adaptive=True, connection_speed=connection_speed
            )
    ...
    def _record_adaptation(
        self,
        download_id: str,
        from_format: MediaFormat,
        to_format: MediaFormat
    ) -> None:
        ...
        self.adaptation_history[download_id].append((from_format.format_id, from_quality))
    ...
    def _get_lower_quality_alternatives(
        ...
        if current_height:
            ...
            alternatives = [
                fmt for fmt in alternatives
                if self._get_format_height(fmt) and self._get_format_height(fmt) <= min_height
            ]
